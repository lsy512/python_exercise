...
#    url='http://quote.stockstar.com/stock/ranklist_a_3_1_'+str(page)+'.html'
    url='http://stock.quote.stockstar.com/info_000725.shtml'
...
#    print('get page',page)                  #打印成功获取的页码
    tt=soup.find_all(id='stock_quoteinfo_xj')
    print (tt[0].text)
#    print (soup.find_all(id='stock_quoteinfo_xj'))
    ...
    time_all = time_all+ time_sleep        #每抓一页随机休眠几秒，数值可根据实际情况改动
    print (time_all)
    shi=time_all/3600
    fen=(time_all%3600)/60
    miao=time_all%60
    print ('run %dh %dm %ds'%(shi,fen,miao))
#删除空白字符
